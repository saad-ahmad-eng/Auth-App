...
| TEST-DEPLOY-001 | ADR-009 | Remote client → cloud VM | Full journey succeeds | **Justified exception (Phase 10):** requires a provisioned cloud VM, which requires OQ-06 (cloud provider choice) resolved — both explicitly scoped to Phase 11 (Implementation.md), not Phase 10. Everything this test would exercise *except* genuine network separation and a public IP — the full login→upload→lock→download→unlock→logout journey, RMI-over-TLS, encryption, locking, audit — is already independently proven locally (see every other row in this matrix, plus Phase 9's two-actor integration session). Deferred, not skipped: tracked as Phase 11's own deliverable. | Not Run (deferred to Phase 11 — justified, see note) |
| TEST-DEPLOY-002 | ADR-009 | Server restart on VM | Resumes service | **Justified exception (Phase 10):** same dependency on Phase 11's cloud VM as TEST-DEPLOY-001. The underlying behavior this test targets — no persistent session/lock state, so a restart cleanly drops both rather than resuming in a corrupt state — is already confirmed by design and by OQ-04 (`SessionManager`/`LockManager` are purely in-memory, confirmed Phase 3/5) and by every local server restart performed across every phase of this project without incident. What specifically remains untested is systemd/`nohup` process supervision on an actual VM, which doesn't exist until Phase 11. | Not Run (deferred to Phase 11 — justified, see note) |

All other rows in this matrix are `Pass`, confirmed fresh in Phase 10 with a from-scratch clean build (`./gradlew clean test`, 88/88 tests, 0 failures/errors across 15 test classes) plus 4 additional full re-runs of the mandatory concurrency test (TEST-CONC-001) — zero winner-collisions across every round. A sample of this matrix's test-method citations was spot-checked directly against the test source to confirm they are not stale or aspirational (Phase 10 consistency audit). TEST-DEPLOY-001/002 are the only `Not Run` rows, both justified exceptions correctly deferred to Phase 11 per Implementation.md's own phase scoping — not oversights.
