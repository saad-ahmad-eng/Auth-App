...
**Status:** Accepted (Implementation Phase 6)
...
**Selected option — implemented as both, resolving OQ-05:**
1. **AES-256-GCM applied to file payloads, per-hop, at the application layer** — the client encrypts before `uploadFile()` and the server decrypts on receipt; the server encrypts (fresh IV) before returning from `downloadFile()` and the client decrypts. Storage stays plaintext (Security.md §7's "at rest" posture is unchanged — a real deployment could layer at-rest encryption on top, tracked as OQ-10, not required by `auth`). Key management: a **pre-shared AES-256 key**, generated by the server on first run and read by the client from a shared local file (`SharedKeyProvider`) — an explicit, documented coursework-scope simplification (no rotation, no per-user keys), not a production KMS.
2. **RMI-over-TLS**, on by default, protecting the whole channel including `login()` credentials and session tokens — the gap application-layer file encryption alone would have left open. Uses a self-signed certificate auto-generated via the JDK-bundled `keytool` (no new dependency) on first run (`DevTlsSetup`), with a fixed, publicly-documented dev-only store password (not a real secret — protects nothing beyond a throwaway local cert).

**Reason:** Together these satisfy `auth`'s explicit requirement using standard, authenticated Java cryptography APIs (LO4), close the credential-in-transit gap that AES-GCM-on-payloads alone would leave, and avoid the custom-crypto prohibition. Doing both was more implementation effort than either alone, but was judged worthwhile specifically because RMI-over-TLS closes a real, otherwise-undocumented-away gap (plaintext credentials) rather than being purely redundant with the payload encryption.
**Trade-offs:** Both key-management schemes (shared AES key file; self-signed dev TLS cert) are coursework-scope simplifications, clearly documented as such in `SharedKeyProvider`/`DevTlsSetup`'s Javadoc and in Security.md §7 — not production-grade key/certificate management. RMI-over-TLS's dev certificate defaults to a SAN scoped to `localhost`/`127.0.0.1`; **resolved in Phase 11:** `DevTlsSetup` accepts `-Dauthlock.tls.extraSan` to extend it with a real deployment address, set automatically by `scripts/provision-vm.sh`/`scripts/setup-authlock.sh` from the VM's discovered public IP/hostname — no manual regeneration step needed.
**Consequences:** Full design in [Security.md](Security.md) §7. Implementation in `authlock-common.crypto` (`AesGcmCipher`, `SharedKeyProvider`, shared by both modules), `authlock-common.tls` (`DevTlsSetup`), and `authlock-server.crypto` (`EncryptionService`). `VaultServiceImpl` gained a `tlsEnabled` constructor parameter; the pre-existing plain-RMI constructor is kept deliberately for the bulk of the test suite (business-logic tests don't need to re-verify the transport layer on every run) — a dedicated `VaultServiceTlsIntegrationTest` proves TLS itself works, including that a non-TLS client is genuinely rejected.
...
**Trade-offs:** No built-in query/search UI — acceptable for coursework demonstration (log can be inspected directly or with simple `grep`/scripting for the report). **Implemented (Phase 7):** hand-rolled JSON serialization (`AuditLogger`) rather than a JSON library dependency — the field set is small and fully known in advance, so this avoids pulling in Jackson/Gson for something this simple. Writes are synchronous and serialized under a single lock (low volume — one line per security-relevant operation, not per byte transferred — so this is not a throughput concern, and it guarantees lines are never interleaved under concurrent RMI calls). A failed write is logged to stderr but does not fail the underlying operation (fail-open on the audit trail, not fail-closed on availability) — a Derived Decision for this coursework project's priorities, documented in `AuditLogger`'s Javadoc.
**Consequences:** Full event schema in [Security.md](Security.md) §9; write path in `authlock-server.audit` (`AuditLogger`, `AuditRecord`, `AuditEventType`). Security.md §9's original "Authentication failure"/"Authorization failure" event rows were merged into each triggering operation's own event type + `FAILURE` result + `ErrorCode` (Derived Decision, Phase 7) rather than kept as separate generic buckets — see Security.md §9 for the reasoning.
...
| ADR-007 | AES-GCM encryption + RMI-over-TLS (both implemented) | Accepted |
