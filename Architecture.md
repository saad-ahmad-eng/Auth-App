...
| **TLS certificate (Phase 6 addition; extended Phase 11)** | The auto-generated dev certificate (`DevTlsSetup`) defaults to `CN=localhost`, SAN `dns:localhost,ip:127.0.0.1` — correct for local development, not for a cloud VM's public IP on its own. **Resolved in Phase 11:** `DevTlsSetup` now accepts `-Dauthlock.tls.extraSan=<entry>[,<entry>...]` (`dns:`/`ip:` `keytool`-style entries), appended to the SAN list the *first* time a given keystore path is generated. `scripts/provision-vm.sh`/`scripts/setup-authlock.sh` set this to the VM's discovered public IP (and public DNS name, on AWS) before the server's first run, so the certificate is correct from the start — no more manual "delete and regenerate" step. |
